...
const classModel = require("../models/sets.model")
...
class UserController {
    ...
    async registerUser(obj) {
        try {
            ...
            let emailFound = await userModel.findOne({ email: userEmail })
            if (emailFound) {
    ...
    async getUser(id) {
        try {
            const user = await userModel.findById(id).populate(["chapter", "set"]).exec();
            if (!user) {
                return { ok: false, message: "User not found" };
            }
            return { ok: true, payLoad: user };
    ...
    async getUsers() {
        try {
            const users = await userModel.find().populate(["set", "chapter"]).exec();
